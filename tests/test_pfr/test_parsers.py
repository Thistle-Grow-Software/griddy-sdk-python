...
class TestParseScheduleTable:
    def test_returns_list_of_schedule_games(
        self, parser: PFRParser, schedule_html: str
    ):
        result = parser.parse_schedule_table(schedule_html)
        assert isinstance(result, list)
        assert len(result) > 0
        assert isinstance(result[0], ScheduleGame)

    def test_game_count(self, parser: PFRParser, schedule_html: str):
        """2015 season: 256 regular-season + 11 playoff = 267 games."""
        games = parser.parse_schedule_table(schedule_html)
        assert len(games) == 267

    def test_first_game_is_week_1(self, parser: PFRParser, schedule_html: str):
        games = parser.parse_schedule_table(schedule_html)
        first = games[0]
    ...
    def test_first_game_links(self, parser: PFRParser, schedule_html: str):
        games = parser.parse_schedule_table(schedule_html)
        first = games[0]
    ...
    def test_last_game_is_super_bowl(self, parser: PFRParser, schedule_html: str):
        games = parser.parse_schedule_table(schedule_html)
        last = games[-1]
    ...
    def test_away_game_has_at_symbol(self, parser: PFRParser, schedule_html: str):
    ...
    def test_home_game_has_empty_location(self, parser: PFRParser, schedule_html: str):
        """NE vs PIT in week 1 (first game) — home game."""
        games = parser.parse_schedule_table(schedule_html)
        first = games[0]
        assert first.game_location == ""

    def test_numeric_fields_are_ints(self, parser: PFRParser, schedule_html: str):
        games = parser.parse_schedule_table(schedule_html)
        first = games[0]
        for field in (
            "pts_win",
            ...
            assert isinstance(getattr(first, field), int), f"{field} should be int"

    def test_thead_separator_rows_excluded(self, parser: PFRParser, schedule_html: str):
        """No game should have 'Week' as its week_num (that's a thead row)."""
        games = parser.parse_schedule_table(schedule_html)
        for game in games:
            assert game.week_num != "Week"

    def test_playoffs_divider_excluded(self, parser: PFRParser, schedule_html: str):
    ...
    def test_all_games_have_required_fields(
        ...
        for i, game in enumerate(games):
            ...
            assert (
                game.game_day_of_week is not None
    ...
    def test_raises_on_missing_table(self, parser: PFRParser):
        with pytest.raises(ValueError, match="Could not find"):
            parser.parse_schedule_table("<html><body>No table here</body></html>")

    def test_super_bowl_neutral_site(self, parser: PFRParser, schedule_html: str):
